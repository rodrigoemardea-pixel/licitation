...
function renderAcomp() {
  ...
  if (!rows.length) {
    ...
    return `<tr class="lb-row" onclick="verAcomp('${r.id}')">
      ...
      <td><div class="lb-cell-inline"><span class="lb-cell-truncate">${r.orgao||'—'}</span>${r.estado ? lbBadgeUF(r.estado) : ''}${r.link ? `}" target="_blank" rel="noopener noreferrer" onclick="event.stopPropagation()" title="Abrir pagina de acompanhamento no sistema" class="lb-cell-link">🔗</a>` : ''}</div></td>
