...
// Implementado neste arquivo para preservar integralmente o carregamento do banco no arquivo 03.
function situacaoPrazoPagamento(r) {
  const compras = (r && r.compras) || [];

  if (!compras.length) {
    return { codigo: 'sem-compras', rotulo: 'SEM COMPRAS', dataInicio: '' };
  }

  if (empenhoEstaPago(r)) {
    return { codigo: 'pago', rotulo: 'PAGO', dataInicio: '' };
  }

  const compraSemStatus = compras.find(c => !c.statusEntrega);
  if (compraSemStatus) {
    return { codigo: 'status-pendente', rotulo: 'STATUS DE ENTREGA PENDENTE', dataInicio: '' };
  }
  ...
  if (compraNaoRecebida) {
    ...
    return {
      codigo: 'entrega-pendente',
      rotulo: rotulos[compraNaoRecebida.statusEntrega] || 'ENTREGA PENDENTE',
      dataInicio: ''
    };
  }

  const compraRecebidaSemData = compras.find(c => !c.dataRecebimentoMercadoria);
  if (compraRecebidaSemData) {
    return { codigo: 'data-pendente', rotulo: 'DATA DE RECEBIMENTO PENDENTE', dataInicio: '' };
  }

  const dataInicio = compras.reduce((maiorData, compra) => {
    return compra.dataRecebimentoMercadoria > maiorData
...
// Sobrescreve somente a regra de cálculo, sem substituir o arquivo 03 nem suas rotinas de banco/cache.
...
// Cria os campos dentro do modal já existente, sem exigir alteração no HTML principal.
...
function abrirCompra(empenhoId, compraId) {
  ...
  ['c-data-recebimento-mercadoria','c-lote-desc','c-qtd-empenho','c-vem-item','c-qtd','c-dcompra','c-plataforma','c-link','c-vunit','c-vtotal-compra','c-custo','c-dpag','c-recebido','c-obs','c-luc-m','c-rec-m'].forEach(id=>{const el=g(id);if(el)el.value='';});
...
function saveCompra() {
  ...
  if (statusEntrega === 'recebida' && !dataRecebimentoMercadoria) {
    ...
    const campoData = g('c-data-recebimento-mercadoria');
